	...
	public static void main(String[] args) {
		...
		for (int k = 0; k < 5; k++) {
			...
			try {
				...
				ArrayList<CompareUnitDouble<TheoricPlace>> places = IAManager.choosePlaceWithFriends(
						TheoricDataBase.mainUser, TheoricDataBase.friends, TheoricDataBase.places, 0.3);

				int i = 0;

				while (i < TheoricDataBase.mainUser.getVisitedRecently().size()
						&& !TheoricDataBase.mainUser.getVisitedRecently().get(i).equals(places.get(i).getElement())) {
					i++;
				}
				...
				System.out.println(
						"Give note : " + note + " expected note : " + MathTools.roundAt(places.get(0).getValue() * 5, 3)
								+ " pref : " + Matrix.trans(TheoricDataBase.mainUser.getPreferences()));
